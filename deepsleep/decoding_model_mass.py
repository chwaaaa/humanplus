...
class DecodingDeepFeatureNet(object):
    ...
    def build_model(self, input_var):
        ...
        network = self._conv1d_layer(input_var=input_var, filter_size=50, n_filters=64, stride=6, wd=1e-3)  # (, 1280, 1, 64)
        # Max pooling
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(name=name, input_var=network, pool_size=8, stride=8)  # (, 160, 1, 64)
        ...
        # Max pooling
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(name=name, input_var=network, pool_size=4, stride=4) # (, 40, 1, 64)
        self.activations.append((name, network))
        self.layer_idx += 1
        ...
        ######### Decoder #########
        latent_vec = self._deconv1d_layer(input_var=latent_vec, filter_size=1, n_filters=128, stride=4, output_shape=160)
        ...
        # upsample
        latent_vec = self._deconv1d_layer(input_var=latent_vec, filter_size=1, n_filters=64, stride=8, output_shape=1280)
        # deconvolution
