...
from deepsleep.decoding_model_ori import DecodingDeepFeatureNet, DecodingDeepSleepNet
...
class DecodingDeepFeatureNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, epoch, network, inputs, targets, train_op, is_train):
        ...
        for x_batch, y_batch in iterate_minibatches(inputs,
            ...
            _, _, loss_st, loss_de, y_pred, x_pred = sess.run(
                [train_op[0], train_op[1], network.staging_loss, network.denoising_loss,network.pred_op, network.denoised_op],
                feed_dict=feed_dict
            )
    ...
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            denoising_vars = tf.trainable_variables()[:13] + denoising_vars
...
class DecodingDeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _, _,_, loss_st, loss_de, y_pred, x_pred, fw_state, bw_state = sess.run(
                    [train_op[0],train_op[1], train_op[2],network.staging_loss,  network.denoising_loss,  network.pred_op, network.denoised_op,network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
                psnr = cal_psnr(x_batch, x_pred)
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            denoising_vars = train_vars1[:13] + denoising_vars
            train_op2, grads_and_vars_op2 = adam(
                loss=train_net.denoising_loss,
                lr=1e-3,
                train_vars=denoising_vars
