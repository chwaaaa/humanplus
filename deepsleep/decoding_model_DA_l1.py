...
class DecodingDeepFeatureNet(object):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            # Build model
            denoised_output, network_ori = self.build_model(input_var=self.input_var)

            # Softmax linear
            name = "l{}_softmax_linear".format(self.layer_idx)
            network = fc(name=name, input_var=network_ori, n_hiddens=self.n_classes, bias=0.0, wd=0)
            self.activations.append((name, network))
            self.layer_idx += 1
            ...
            # Softmax linear for target domain
            name = "l{}_softmax_linear_".format(self.layer_idx)
            network = fc(name=name, input_var=network_ori, n_hiddens=self.n_classes, bias=0.0, wd=0)
            self.activations.append((name, network))
            self.layer_idx += 1
            
            self.logits_tar = network
            ...
            # denoising loss
            # FIXME
            denoising_loss = tf.abs(denoised_output - self.input_var)
            ...
            # Cross-entropy loss
            staging_loss_tar = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits_tar,
                labels=self.target_var,
                name="sparse_softmax_cross_entropy_with_logits"
            )
            self.staging_loss_tar = staging_loss_tar
...
class DecodingDeepSleepNet(DecodingDeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            staging_loss_tar = tf.contrib.legacy_seq2seq.sequence_loss_by_example(
                [self.logits_tar],
                [self.target_var],
                [tf.ones([self.batch_size * self.seq_length])],
                name="sequence_loss_by_example"
            )
            self.staging_loss_tar = tf.reduce_sum(staging_loss_tar) / self.batch_size
