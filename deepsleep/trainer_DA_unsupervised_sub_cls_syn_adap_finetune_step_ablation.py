...
class DeepFeatureNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, subjects, inputs_unseen, targets_unseen, train_op, alpha, beta, gamma, is_train, m=1):
        ...
        if is_train:
            ...
            class_tar = int(len(targets_unseen) / 5 *m )
            inputs, targets, subjects = get_balance_class_subject_by_target(inputs, targets, subjects, class_tar)
            if len(targets) >=len(targets_unseen):
                for (x_batch, y_batch, s_batch), (x_batch_unseen, y_batch_unseen) in zip(itertools.cycle(iterate_minibatches_subject(inputs, targets,subjects, self.batch_size, shuffle=is_shuffle)), iterate_minibatches(inputs_unseen, targets_unseen, self.batch_size, shuffle=is_shuffle)):
                ...
                for (x_batch, y_batch, s_batch), (x_batch_unseen, y_batch_unseen) in zip(
                        iterate_minibatches_subject(inputs, targets, subjects, self.batch_size, shuffle=is_shuffle),
                        itertools.cycle(iterate_minibatches(inputs_unseen, targets_unseen, self.batch_size,
                                                            shuffle=is_shuffle))):
                    feed_dict = {
                        network.input_var: x_batch,
                        network.is_source: True,
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                y_true_val, y_pred_val, y_true_val_unseen, y_pred_val_unseen, src_valid_losses, tar_valid_losses, valid_duration, _, _, _, src_valid_n_batches, tar_valid_n_batches= \
                    self._run_epoch(
                        ...
                        is_train=False, m=self.ablation
                    )
...
class DeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, subjects, inputs_unseen, targets_unseen, train_op, alpha, beta, is_train, m=1):
        ...
        if is_train:
            ...
            inputs, targets, subjects = inputs[:int(len(inputs)*m)], targets[:int(len(inputs)*m)], subjects[:int(len(inputs)*m)]

            if len(inputs) >= len(inputs_unseen):
                for (sub_idx, each_data), (sub_idx_unseen, each_data_unseen) in zip(enumerate(itertools.cycle(itertools.izip(inputs, targets, subjects))), enumerate(itertools.izip(inputs_unseen, targets_unseen))):
                ...
                for (sub_idx, each_data), (sub_idx_unseen, each_data_unseen) in zip(
                        enumerate(itertools.izip(inputs, targets, subjects)),
                        enumerate(itertools.cycle(itertools.izip(inputs_unseen, targets_unseen)))):
                    each_x, each_y, each_s = each_data
                    each_x_unseen, each_y_unseen = each_data_unseen
                    ...
                    for (x_batch, y_batch, s_batch), (x_batch_unseen, y_batch_unseen) in zip(
                            iterate_batch_seq_minibatches_subject(inputs=each_x, targets=each_y, subjects=each_s,
                                                                  batch_size=self.batch_size,
