...
class DeepFeatureNet(object):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            # Cross-entropy dual loss
            self.target_var_onehot  = tf.one_hot(self.target_var, depth = self.n_classes)
            dual_loss = - tf.losses.softmax_cross_entropy((1 - self.target_var_onehot), tf.reduce_sum(tf.exp(self.logits)) + self.logits)
            dual_loss = tf.reduce_mean(dual_loss)
            self.dual_loss = dual_loss
            
            # Regularization loss
            self.regular_loss = tf.add_n(
                tf.get_collection("losses", scope=scope.name + "\/")[:-1],
                name="regular_loss"
            )
            # Total loss
            self.loss_op = self.loss*5 + self.regular_loss + self.dual_loss
            #self.loss_op = tf.add(self.loss_op, self.dual_loss)
...
class DeepSleepNet(DeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            self.target_var_onehot  =tf.one_hot(self.target_var, depth = self.n_classes)
            dual_loss = - tf.losses.softmax_cross_entropy((1-self.target_var_onehot), tf.reduce_sum(tf.exp(self.logits)) + self.logits)
            self.dual_loss = tf.reduce_sum(dual_loss) / self.batch_size
            ...
            # Total loss
            self.loss_op = self.loss *5 + regular_loss +self.dual_loss
...
class DecodingDeepFeatureNet(object):
    ...
    def _deconv1d_layer(self, input_var, filter_size, n_filters, stride, output_shape = None, wd=0, activation='relu',bn=True):
        ...
        name = "l{}_conv".format(self.layer_idx)
        with tf.variable_scope(name) as scope:
            output = deconv_1d(name="conv1d", input_var=input_var, filter_shape=[filter_size, 1, n_in_filters, n_filters],
...
class DecodingDeepSleepNet(DeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            # Build model
            denoising_output, network = self.build_model(input_var=self.input_var)
