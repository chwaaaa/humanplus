...
from deepsleep.model_multi import DeepFeatureNet, DeepSleepNet
#, DecodingDeepSleepNet
#from deepsleep.denoise_model import DenoisingDeepFeatureNet, DenoisingDeepSleepNet
...
class DeepSleepNetTrainer(Trainer):
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume,  transfer=False, transfer_dir = None):
        if transfer:
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            if transfer:
                train_vars1 = tf.trainable_variables()[:2]
                train_vars2 = tf.trainable_variables()[2:]
            ...
            load_pretrain = False
            if transfer:
                output_dir = os.path.join(transfer_dir,  train_net.name)
                ...
                if os.path.exists(output_dir):
                    if os.path.isfile(os.path.join(output_dir, "checkpoint")):
                        try:
                            ...
                            saver.restore(sess, output_dir + "/model_fold0.ckpt-50")
                ...
                for idx, data_loader in enumerate(data_loaders):
                    ...
                    if idx >= 1:
                        for each_idx, x_t  in enumerate(x_tr):
                            x_train[each_idx] = np.concatenate([x_train[each_idx], x_t], axis=-1)
                        for each_idx, x_v in enumerate(x_val):
                            x_valid[each_idx] = np.concatenate([x_valid[each_idx], x_v], axis=-1)
...
class DecodingDeepSleepNetTrainer(Trainer):
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        pretrained_model_name = "decodingdeepsleepnet"
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            if sess.run(global_step) < n_epochs:
                ...
                x_train  = np.concatenate(x_train, axis=-1)
                x_valid = np.concatenate(y_valid, axis=-1)
                y_train = y_tr
                y_valid = y_var
                # Performance history
                all_train_loss = np.zeros(n_epochs)
...
class DenoisingDeepFeatureNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for x_batch, y_batch in iterate_minibatches(inputs,
            ...
            _, loss_value, y_pred, x_denoised = sess.run(
        ...
        total_y_pred = np.hstack(y)
        total_y_true = np.hstack(y_true)
        total_x_denoised = np.hstack(x_)
        total_x_true = np.hstack(x_batch)

        return total_y_true, total_y_pred, total_x_true, total_x_denoised, total_loss, duration
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.Session() as sess:
            # Build training and validation networks
            train_net = DenoisingDeepFeatureNet(
                sigma=self.sigma,
                batch_size=self.batch_size,
                input_dims=self.input_dims,
                n_classes=self.n_classes,
            ...
            valid_net = DenoisingDeepFeatureNet(
                sigma=self.sigma,
                batch_size=self.batch_size,
                input_dims=self.input_dims,
                n_classes=self.n_classes,
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                # Update parameters and compute loss of training set
                y_true_train, y_pred_train, x_true_train, x_denoised_train, train_loss, train_duration = \
                    self._run_epoch(
                        sess=sess, network=train_net,
                        inputs=x_train, targets=y_train,
                        train_op=train_op,
                ...
                train_acc = np.mean(y_true_train == y_pred_train)
                train_f1 = f1_score(y_true_train, y_pred_train, average="macro")
                #train_psnr = cal_psnr(x_true_train, x_denoised_train)
                # # MONITORING
                # print "AFTER TRAINING and BEFORE VALID"
                # for n in monitor_vars:
                #     v = tf.get_default_graph().get_tensor_by_name(n)
                ...
                # Evaluate the model on the validation set
                y_true_val, y_pred_val, x_true_val, x_denoised_val,valid_loss, valid_duration = \
                    self._run_epoch(
                        sess=sess, network=valid_net,
                        inputs=x_valid, targets=y_valid,
                        train_op=tf.no_op(),
                ...
                valid_acc = np.mean(y_true_val == y_pred_val)
                valid_f1 = f1_score(y_true_val, y_pred_val, average="macro")
                #valid_psnr = cal_psnr(x_true_val, x_denoised_val)
                # db.train_log(args={
                #     "n_folds": self.n_folds,
                #     "fold_idx": self.fold_idx,
                #     "epoch": epoch,
                ...
                #print "train psnr = {}, valid psnr = {}".format(train_psnr, valid_psnr)
...
class DenoisingDeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _, loss_value, y_pred, x_denoised, fw_state, bw_state = sess.run(
                    [train_op, network.loss_op, network.pred_op,network.denoised_op,  network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            # Build training and validation networks
            train_net =DenoisingDeepSleepNet(
                batch_size=self.batch_size,
                sigma=self.sigma,
                input_dims=self.input_dims,
                n_classes=self.n_classes,
            ...
            valid_net = DenoisingDeepSleepNet(
                batch_size=self.batch_size,
                sigma = self.sigma,
                input_dims=self.input_dims,
                n_classes=self.n_classes,
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                # Update parameters and compute loss of training set
                y_true_train, y_pred_train,x_true_train, x_pred_train, train_loss, train_duration = \
                    self._run_epoch(
                        sess=sess, network=train_net,
                        inputs=x_train, targets=y_train,
                        train_op=train_op,
                ...
                train_acc = np.mean(y_true_train == y_pred_train)
                train_f1 = f1_score(y_true_train, y_pred_train, average="macro")
                #train_psnr = cal_psnr(x_true_train, x_pred_train)
                # Evaluate the model on the validation set
                y_true_val, y_pred_val, x_true_val, x_pred_val, valid_loss, valid_duration = \
                    self._run_epoch(
                        sess=sess, network=valid_net,
                        inputs=x_valid, targets=y_valid,
                        train_op=tf.no_op(),
                ...
                valid_acc = np.mean(y_true_val == y_pred_val)
                valid_f1 = f1_score(y_true_val, y_pred_val, average="macro")
                #valid_psnr = cal_psnr(x_true_val, x_pred_val)
                all_train_loss[epoch] = train_loss
                all_train_acc[epoch] = train_acc
                all_train_f1[epoch] = train_f1
                #all_train_psnr[epoch] =train_psnr
