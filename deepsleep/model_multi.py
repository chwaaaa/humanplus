...
class DeepFeatureNet(object):
    ...
    def _conv1d_layer(self, input_var, filter_size, n_filters, stride, wd=0):
        ...
        with tf.variable_scope(name) as scope:
            output = conv_1d(name="conv1d", input_var=input_var, filter_shape=[filter_size, self.input_chs, n_in_filters, n_filters], stride=stride, bias=None, wd=wd)
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            dual_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits= self.logits,
                labels= 1 - self.target_var,
                name="sparse_softmax_cross_entropy_with_logits"
            )
            dual_loss = -tf.reduce_mean(dual_loss, name="dual_cross_entropy")
...
class DeepSleepNet(DeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            # Cross-entropy loss
            dual_loss = tf.contrib.legacy_seq2seq.sequence_loss_by_example(
                [self.logits],
                [1 - self.target_var],
                [tf.ones([self.batch_size * self.seq_length])],
                name="sequence_loss_by_example"
            )
            dual_loss = -tf.reduce_sum(dual_loss) / self.batch_size
