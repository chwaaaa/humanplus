...
def get_balance_class_oversample_subject(x, y,z):
    """
    Balance the number of samples of all classes by (oversampling):
        1. Find the class that has the largest number of samples
        2. Randomly select samples in each class equal to that largest number
    ...
    for c in class_labels:
        ...
        tmp_x = np.repeat(x[idx], n_repeats, axis=0)
        tmp_y = np.repeat(y[idx], n_repeats, axis=0)
        tmp_z = np.repeat(z[idx], n_repeats, axis=0)
        n_remains = n_max_classes - len(tmp_x)
        if n_remains > 0:
            sub_idx = np.random.permutation(idx)[:n_remains]
            tmp_x = np.vstack([tmp_x, x[sub_idx]])
            tmp_y = np.hstack([tmp_y, y[sub_idx]])
            tmp_z = np.hstack([tmp_z, y[sub_idx]])
        balance_x.append(tmp_x)
        balance_y.append(tmp_y)
        balance_z.append(tmp_z)
    balance_x = np.vstack(balance_x)
    balance_y = np.hstack(balance_y)
    balance_z = np.hstack(balance_z)

    return balance_x, balance_y, balance_z

def get_balance_class_subject_by_target(x, y, z, n_classes):
    """n_max_classes
    Balance the number of samples of all classes by (oversampling):
        1. Find the class that has the largest number of samples
        2. Randomly select samples in each class equal to that largest number
    ...
    '''
    for c in class_labels:
        n_samples = len(np.where(src_y == c)[0])
        if n_min_classes > n_samples:
            n_min_classes = n_samples
    '''
    balance_x = []
    balance_y = []
    ...
    for c in class_labels:
        ...
        if len(idx) > n_classes:
            idx = np.random.permutation(idx)[:n_classes]
            balance_x.append(x[idx])
            balance_y.append(y[idx])
            balance_z.append(z[idx])
        # upsample
        else:
            n_repeats = int(n_classes / len(idx))
            tmp_x = np.repeat(x[idx], n_repeats, axis=0)
            tmp_y = np.repeat(y[idx], n_repeats, axis=0)
            tmp_z = np.repeat(z[idx], n_repeats, axis=0)
            n_remains = n_classes - len(tmp_x)
            if n_remains > 0:
                sub_idx = np.random.permutation(idx)[:n_remains]
                tmp_x = np.vstack([tmp_x, x[sub_idx]])
                tmp_y = np.hstack([tmp_y, y[sub_idx]])
...
def iterate_minibatches_subject(inputs, targets, subjects, batch_size, shuffle=False):
    """
    Generate a generator that return a batch of inputs and targets.
    """
    assert len(inputs) == len(targets)
    ...
    for start_idx in range(0, len(inputs) - batch_size + 1, batch_size):
        if shuffle:
            ...
            excerpt = slice(start_idx, start_idx + batch_size)
        yield inputs[excerpt], targets[excerpt], subjects[excerpt]
...
def iterate_batch_seq_minibatches_subject(inputs, targets, subjects, batch_size, seq_length):
    # pdb.set_trace()
    # assert len(inputs) == len(targets)
    n_inputs = len(inputs)
    batch_len = n_inputs // batch_size
    ...
    seq_targets = np.zeros((batch_size, batch_len) + targets.shape[1:],
                           dtype=targets.dtype)
    seq_subjects = np.zeros((batch_size, batch_len) + subjects.shape[1:],
                           dtype=subjects.dtype)

    for i in range(batch_size):
        seq_inputs[i] = inputs[i * batch_len:(i + 1) * batch_len]
        seq_targets[i] = targets[i * batch_len:(i + 1) * batch_len]
        seq_subjects[i] = subjects[i * batch_len:(i + 1) * batch_len]

    #
    for i in range(epoch_size):
        x = seq_inputs[:, i * seq_length:(i + 1) * seq_length]
        y = seq_targets[:, i * seq_length:(i + 1) * seq_length]
        z = seq_subjects[:, i * seq_length:(i + 1) * seq_length]
        flatten_x = x.reshape((-1,) + inputs.shape[1:])
        flatten_y = y.reshape((-1,) + targets.shape[1:])
        flatten_z = z.reshape((-1,) + subjects.shape[1:])
