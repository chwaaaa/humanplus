...
from deepsleep.denoise_model import DenoisingNet, DenoisingDeepFeatureNet, DenoisingDeepSleepNet
...
class DenoisingNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for x_batch, y_batch in iterate_minibatches(inputs,
            ...
            #x_batch +=  np.random.normal(0.0, scale=sigma)
            feed_dict = {
            ...
            #         )
            _, loss_value, x_predict = sess.run(
                [train_op, network.loss_op, network.denoised_op],
                feed_dict=feed_dict
            )
            ...
            n_batches += 1
            psnr = cal_psnr(x_batch, x_predict)
            total_psnr += psnr
            #x_denoised.append(x_predict)
        ...
        #total_x_pred = np.hstack(x_denoised)
        #total_x_true = np.hstack(x_true)

        return total_loss, total_psnr, duration
    ...
    def train(self, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session(config=config) as sess:
            ...
            if resume:
                if os.path.exists(output_dir):
                    if os.path.isfile(os.path.join(output_dir, "checkpoint")):
                        ...
                        print "[{}] Resume pre-pre-training ...\n".format(datetime.now())
                    else:
                        print "[{}] Start pre-pre-training ...\n".format(datetime.now())
            else:
                print "[{}] Start pre-pre-training ...\n".format(datetime.now())
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                train_loss, train_psnr, train_duration = \
                    self._run_epoch(
                ...
                valid_loss, valid_psnr, valid_duration = \
                    self._run_epoch(
                ...
                self.print_denoising_performance(
                    ...
                    epoch, n_epochs,
                    train_duration, train_loss, train_psnr,
                    valid_duration, valid_loss, train_psnr
                )
...
class DenoisingDeepFeatureNetTrainer(Trainer):
    ...
    def train(self, pretrained_model_path, n_epochs, resume):
        pretrained_model_name= "denoisingnet"
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True
        with tf.Graph().as_default(), tf.Session(config=config) as sess:
            # Build training and validation networks
...
class DenoisingDeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _, loss_value, y_pred, x_pred, fw_state, bw_state = sess.run(
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        pretrained_model_name = "denoisingdeepfeaturenet"
