...
from deepsleep.decoding_model_mass import DecodingDeepFeatureNet, DecodingDeepSleepNet
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _, _, loss_value, y_pred, x_pred, fw_state, bw_state = sess.run(
                    [train_op[0],train_op[1], network.loss_op, network.pred_op, network.denoised_op,network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
                psnr = cal_psnr(x_batch, x_pred)
