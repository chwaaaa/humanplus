...
from deepsleep.decoding_model_DA import DecodingDeepFeatureNet, DecodingDeepSleepNet
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _,  loss_de,  x_pred, fw_state, bw_state = sess.run(
                    [train_op[2], network.denoising_loss, network.denoised_op, network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
