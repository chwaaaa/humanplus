...
from deepsleep.model_dual_loss import DeepFeatureNet, DeepSleepNet
...
class Trainer(object):
    ...
    def print_performance(self, sess, output_dir, network_name,
                           ...
                           train_duration, train_st_loss, train_dual_loss, train_acc, train_f1,
                           valid_duration, valid_st_loss, valid_dual_loss, valid_acc, valid_f1):
        # Get regularization loss
        train_reg_loss = tf.add_n(tf.get_collection("losses", scope=network_name + "\/")[:-1])
        ...
        if ((epoch + 1) % self.interval_print_cm == 0) or ((epoch + 1) == n_epochs):
            ...
            print (
                "train ({:.3f} sec): n={}, st_loss={:.3f} dual_loss={:.3f} ({:.3f}), acc={:.3f}, "
                "f1={:.3f}".format(
                    train_duration, n_train_examples,
                    train_st_loss, train_dual_loss, train_reg_loss_value,
                    train_acc, train_f1
                )
            )
            print train_cm
            print (
                "valid ({:.3f} sec): n={}, st_loss={:.3f} dual_loss={:.3f} ({:.3f}), acc={:.3f}, "
                "f1={:.3f}".format(
                    valid_duration, n_valid_examples,
                    valid_st_loss, valid_dual_loss, valid_reg_loss_value,
                    valid_acc, valid_f1
                )
            )
            print valid_cm
            ...
            print (
                "epoch {}: "
                "train ({:.2f} sec): n={}, st_loss={:.3f} dual_loss={:.3f} ({:.3f}), "
                "acc={:.3f}, f1={:.3f} | "
                "valid ({:.2f} sec): n={}, st_loss={:.3f} dual_loss={:.3f} ({:.3f}), "
                "acc={:.3f}, f1={:.3f}".format(
                    epoch+1,
                    train_duration, n_train_examples,
                    train_st_loss, train_dual_loss,train_reg_loss_value,
...
class DeepFeatureNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        y = []
        y_true = []
        total_loss, total_dual_loss,  n_batches = 0.0,0.0, 0
        ...
        for x_batch, y_batch in iterate_minibatches(inputs,
            ...
            _, loss_value, dual_loss_value, y_pred, dual_loss_value, logits = sess.run(
                [train_op, network.loss, network.dual_loss, network.pred_op, network.dual_loss, network.logits],
                feed_dict=feed_dict
            )

            total_loss += loss_value
            total_dual_loss+=dual_loss_value
            n_batches += 1
            y.append(y_pred)
        ...
        total_dual_loss /= n_batches
        total_y_pred = np.hstack(y)
        total_y_true = np.hstack(y_true)
        return total_y_true, total_y_pred, total_loss, total_dual_loss, duration
    ...
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                y_true_train, y_pred_train, train_loss,train_dual_loss, train_duration = \
                    self._run_epoch(
                ...
                y_true_val, y_pred_val, valid_loss, valid_dual_loss, valid_duration = \
                    self._run_epoch(
                ...
                self.print_performance(
                    ...
                    n_train_examples, n_valid_examples,
                    train_cm, valid_cm, epoch, n_epochs,
                    train_duration, train_loss, train_dual_loss, train_acc, train_f1,
                    valid_duration, valid_loss, valid_dual_loss, valid_acc, valid_f1
                )
    ...
    def _run_epoch(self, sess, network, inputs, targets, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                _, loss_value, dual_loss_value, y_pred, fw_state, bw_state = sess.run(
                    [train_op, network.loss, network.dual_loss, network.pred_op, network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
