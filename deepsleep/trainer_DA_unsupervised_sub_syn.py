...
from deepsleep.utils import iterate_minibatches, iterate_batch_seq_minibatches, iterate_batch_seq_minibatches_subject
...
class DeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, subjects, inputs_unseen, targets_unseen, train_op, is_train):
        start_time = time.time()
        y = []
        y_true = []
        ...
        return total_y_true, total_y_pred, total_y_true_unseen, total_y_pred_unseen, [src_total_loss_st, src_total_loss_do, src_total_loss_cl, src_total_loss_sub, src_total_loss_syn], [tar_total_loss_st, tar_total_loss_do, tar_total_loss_cl, tar_total_loss_sub,  tar_total_loss_syn], duration
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            # Define optimization operations
            train_op1, grads_and_vars_op1 = adam(
                loss= train_net.domain_loss + self.beta*(train_net.syn_loss),
                lr=1e-4,
                train_vars=domain_vars
            )
            
            train_op2, grads_and_vars_op2 = adam(
                loss= train_net.class_distributor_loss + self.beta*(train_net.syn_loss),
                lr=1e-4,
                train_vars = class_vars
            )
            
            train_op3, grads_and_vars_op3 = adam(
                loss= train_net.subject_discriminator_loss + self.beta*(train_net.syn_loss),
                lr=1e-4,
            ...
            train_op5, grads_and_vars_op5 = adam(
                loss=train_net.staging_loss + self.beta*(train_net.syn_loss) - (train_net.domain_loss + train_net.class_distributor_loss + train_net.subject_discriminator_loss),
                lr= 1e-4,
                train_vars=feature_vars
            )
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                y_true_val, y_pred_val, y_true_val_unseen, y_pred_val_unseen, src_valid_losses, tar_valid_losses, valid_duration = \
                    self._run_epoch(
                        ...
                        inputs_unseen=unseen_x_valid, targets_unseen=unseen_y_valid,
                        train_op=[tf.no_op(), tf.no_op(), tf.no_op(), tf.no_op(), tf.no_op()],
                        is_train=False
                    )
