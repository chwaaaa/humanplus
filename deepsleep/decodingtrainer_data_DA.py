...
MASS_DATA_DIR = '/mnt/humanplus/data/MASS/EOG Left Horiz/SS3/'

from deepsleep.decoding_model_DA_l1 import DecodingDeepFeatureNet, DecodingDeepSleepNet
...
class DecodingDeepFeatureNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, epoch, network, inputs, targets, inputs_unseen, targets_unseen, train_op, is_train):
        ...
        for (x_batch, y_batch) in iterate_minibatches(inputs, targets, self.batch_size,shuffle=is_shuffle):
            ...
            x_batch_unseen, y_batch_unseen = iterate_minibatches(inputs_unseen, targets_unseen, self.batch_size, shuffle=is_shuffle).next()
            #downsample unseen data
            x_batch_unseen_ds = np.zeros_like(x_batch)
            for idx, x_unseen in enumerate(x_batch_unseen):
                x_batch_unseen_ds[idx] = sps.resample(x_unseen, x_batch.shape[1])
                   
            # decoder for unseen signal
            sigma = 0.0
            feed_dict = {
                network.input_var: x_batch_unseen_ds,
                network.sigma: sigma,
                network.target_var: y_batch_unseen
            }

            _,  loss_de, x_pred, y_pred_unseen = sess.run(
                [train_op[1], network.denoising_loss, network.denoised_op, network.pred_op],
                feed_dict=feed_dict
            )
        ...
        return total_y_true, total_y_pred, total_y_true_unseen, total_y_pred_unseen, total_loss_st, total_loss_de, total_psnr, duration
    ...
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            denoising_vars = [var for var in tf.trainable_variables() if var not in tf.trainable_variables()[-4:-2]]
            
            train_op2, grads_and_vars_op2 = adam(
                loss=0.3* train_net.denoising_loss +0.7* train_net.staging_loss_tar,
                lr=1e-4,
...
class DecodingDeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets,inputs_unseen, targets_unseen, train_op, is_train):
        ...
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            each_x_batch_unseen, each_y_batch_unseen = itertools.izip(inputs_unseen, targets_unseen).next()
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                x_batch_unseen, y_batch_unseen = iterate_batch_seq_minibatches(inputs=each_x_batch_unseen,targets=each_y_batch_unseen,
                                              batch_size=self.batch_size,seq_length=self.seq_length).next()
                ...
                # decoder for unseen signal
                feed_dict[network.input_var]=x_batch_unseen_ds
                feed_dict[network.target_var]=y_batch_unseen
                    
                _,  loss_de,  x_pred, y_pred_unseen,fw_state, bw_state = sess.run(
                    [train_op[2], network.denoising_loss, network.denoised_op, network.pred_op,network.fw_final_state, network.bw_final_state],
                    feed_dict=feed_dict
                )
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            staging_vars2 = list(set(tf.trainable_variables()) - set(train_vars1))
            ...
            denoising_vars = tf.trainable_variables()[:-2]
            train_op2, grads_and_vars_op2 = adam(
            ...
            if sess.run(global_step) < n_epochs:
                ...
                unseen_data_loader = mass_SeqDataLoader(
                    data_dir=MASS_DATA_DIR,
                    n_folds=31,
                    fold_idx=0
                )
