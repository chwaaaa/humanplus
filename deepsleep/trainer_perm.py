...
def pgd_linf_untargeted_mostlikely_defense(model, sess, feed_dict, epsilon=0.2, stepsize=0.1, num_iter=20,
                                           randomize=False):
...
def pgd_linf_untargeted_mostlikely_attack(model, sess, feed_dict, epsilon=0.1, stepsize=0.1, num_iter=20,
                                          randomize=False):
    """ Construct FGSM adversarial examples on the examples X"""
    x = feed_dict[model.input_var]
    epsilon = np.max(x) / 2
