...
class Trainer(object):
    ...
    def print_performance(self, sess, network_name,
                          ...
                          train_duration, src_train_loss_st, src_train_loss_do, src_train_loss_cl, src_train_acc, src_train_f1,
                          tar_train_loss_st, tar_train_loss_do, tar_train_loss_cl, tar_train_acc, tar_train_f1,
                          valid_duration, src_valid_loss_st, src_valid_loss_do, src_valid_loss_cl, src_valid_acc, src_valid_f1,
                          tar_valid_loss_st, tar_valid_loss_do, tar_valid_loss_cl, tar_valid_acc, tar_valid_f1):
        ...
        if ((epoch + 1) % self.interval_print_cm == 0) or ((epoch + 1) == n_epochs):
            ...
            print (
                "soruce train ({:.3f} sec): n={}, st_loss={:.3f} do_loss={:.3f}, cl_loss={:.3f}, acc={:.3f}, "
                "f1={:.3f}".format(
                    train_duration, src_n_train_examples,
                    src_train_loss_st, src_train_loss_do, src_train_loss_cl,
                    src_train_acc, src_train_f1
                )
            )
            print src_train_cm
            print (
                "target train ({:.3f} sec): n={}, st_loss={:.3f} do_loss={:.3f}, cl_loss={:.3f}, acc={:.3f}, "
                "f1={:.3f}".format(
                    train_duration, tar_n_train_examples,
                    tar_train_loss_st, tar_train_loss_do, tar_train_loss_cl,
            ...
            print (
                "soruce valid ({:.3f} sec): n={}, st_loss={:.3f} do_loss={:.3f}, cl_loss={:.3f}, acc={:.3f}, "
                "f1={:.3f}".format(
                    valid_duration, src_n_valid_examples,
                    src_valid_loss_st, src_valid_loss_do, src_valid_loss_cl,
                    src_valid_acc, src_valid_f1
                )
            )
            print src_valid_cm
            print (
                "target valid ({:.3f} sec): n={}, st_loss={:.3f} do_loss={:.3f}, cl_loss={:.3f}, acc={:.3f}, "
                "f1={:.3f}".format(
                    valid_duration, tar_n_valid_examples,
                    tar_valid_loss_st, tar_valid_loss_do, tar_valid_loss_cl,
            ...
            print (
                "epoch {}: "
                "source train ({:.2f} sec): n={}, st_loss={:.3f} do_loss={:.3f} cl_loss={:.3f}, "
                "acc={:.3f}, f1={:.3f} | "
                "source valid ({:.2f} sec): n={}, st_loss={:.3f} do_loss={:.3f} cl_loss={:.3f},"
                "acc={:.3f}, f1={:.3f} | "
                "target train ({:.2f} sec): n={}, st_loss={:.3f} do_loss={:.3f} cl_loss={:.3f}, "
                "acc={:.3f}, f1={:.3f} | "
                "target valid ({:.2f} sec): n={}, st_loss={:.3f} do_loss={:.3f} cl_loss={:.3f},"
                "acc={:.3f}, f1={:.3f}".format(
                    epoch + 1,
                    train_duration, src_n_train_examples,
                    src_train_loss_st, src_train_loss_do, src_train_loss_cl, src_train_acc, src_train_f1,
                    valid_duration, src_n_valid_examples,
                    src_valid_loss_st, src_valid_loss_do, src_valid_loss_cl, src_valid_acc, src_valid_f1,
                    train_duration, tar_n_train_examples,
                    tar_train_loss_st, tar_train_loss_do, tar_train_loss_cl, tar_train_acc, tar_train_f1,
                    valid_duration, tar_n_valid_examples,
                    tar_valid_loss_st, tar_valid_loss_do, tar_valid_loss_cl, tar_valid_acc, tar_valid_f1
...
class DeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, inputs_unseen, targets_unseen, train_op, is_train):
        ...
        y = []
        y_true = []
        src_total_loss_st, src_total_loss_do, src_total_loss_cl,  source_n_batches= 0.0, 0.0, 0.0, 0
        # for unseen signal
        y_unseen = []
        y_unseen_true = []
        tar_total_loss_st, tar_total_loss_do, tar_total_loss_cl,  target_n_batches= 0.0, 0.0, 0.0, 0
        for sub_idx, each_data in enumerate(itertools.izip(inputs, targets)):
            ...
            for (x_batch, y_batch) in iterate_batch_seq_minibatches(inputs=each_x, targets=each_y, batch_size=self.batch_size, seq_length=self.seq_length):
                ...
                _, _,  _, src_loss_st, src_loss_do, src_loss_cl, y_pred= sess.run(
                    [train_op[0], train_op[1],train_op[2], network.staging_loss, network.domain_loss, network.class_distributor_loss, network.pred_op],
                    feed_dict=feed_dict
                )
        ...
        for sub_idx_unseen, each_data_unseen in enumerate(itertools.izip(inputs_unseen, targets_unseen)):
            ...
            for x_batch_unseen, y_batch_unseen in iterate_batch_seq_minibatches(inputs=each_x_unseen, targets=each_y_unseen,
                ...
                _, _, _, loss_st, loss_do, loss_cl, y_pred_unseen = sess.run(
                    [train_op[0], train_op[1], train_op[2], network.staging_loss, network.domain_loss, network.class_distributor_loss, network.pred_op],
                    feed_dict=feed_dict
                )
        ...
        return total_y_true, total_y_pred, total_y_true_unseen, total_y_pred_unseen, [src_total_loss_st, src_total_loss_do, src_total_loss_cl], [tar_total_loss_st, tar_total_loss_do, tar_total_loss_cl], duration
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            # Make subdirectory for pretraining
            output_dir = os.path.join(self.output_dir, "fold{}".format(self.source_fold_idx), train_net.name)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                self.print_performance(
                    ...
                    src_n_train_examples, src_n_valid_examples, tar_n_train_examples, tar_n_valid_examples,
                    src_train_cm, src_valid_cm, tar_train_cm, tar_valid_cm, epoch, n_epochs,
                    train_duration, src_train_losses[0], src_train_losses[1], src_train_losses[2], src_train_acc, src_train_f1,
                    tar_train_losses[0], tar_train_losses[1], tar_train_losses[2], tar_train_acc, tar_train_f1,
                    valid_duration, src_valid_losses[0], src_valid_losses[1], src_valid_losses[2], src_valid_acc, src_valid_f1,
                    tar_valid_losses[0], tar_valid_losses[1], tar_valid_losses[2], tar_valid_acc, tar_valid_f1
                )
                '''
                # Save performance history
                np.savez(
                    os.path.join(output_dir, "perf_fold{}.npz".format(self.source_fold_idx)),
                    src_train_loss_st=all_train_loss_st, 
                    src_valid_loss_st=all_valid_loss_st,
                    src_train_acc=all_train_acc, src_valid_acc=all_valid_acc,
                    src_train_f1=all_train_f1, src_valid_f1=all_soruce_valid_f1,
                    y_true_val=np.asarray(y_true_val),
                    y_pred_val=np.asarray(y_pred_val)
                )
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    start_time = time.time()
                    save_path = os.path.join(
                        output_dir, "model_fold{}.ckpt".format(self.source_fold_idx)
                    )
                    saver.save(sess, save_path, global_step=global_step)
                    duration = time.time() - start_time
                    print "Saved model checkpoint ({:.3f} sec)".format(duration)
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    ...
                    np.savez(
                        os.path.join(
                            output_dir,
                            "params_fold{}.npz".format(self.source_fold_idx)),
                        **save_dict
                    )
                    duration = time.time() - start_time
                    print "Saved trained parameters ({:.3f} sec)".format(duration)
        ...
        return os.path.join(output_dir, "params_fold{}.npz".format(self.source_fold_idx))
