...
def deconv_1d(name, input_var, filter_shape, stride, output_shape = None, padding="SAME",
            bias=None, wd=None):
    with tf.variable_scope(name) as scope:
        batch_size = input_var.get_shape()[0]
        if output_shape is None:
            output_shape = input_var.get_shape()[1]*stride
        output_channel = filter_shape[-2]
        # Trainable parameters
        kernel = variable_with_weight_decay(
            "weights",
            shape=filter_shape,
        ...
        # Convolution
        output_var = tf.nn.conv2d_transpose(
            input_var,
            kernel,
            output_shape = [batch_size, output_shape, 1, output_channel],
