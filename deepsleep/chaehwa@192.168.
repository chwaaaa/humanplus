...
from deepsleep.model_DA_unsupervised_multi import DeepFeatureNet, DeepSleepNet
...
class DeepFeatureNetTrainer(Trainer):
    ...
    def __init__(
        ...
        self.input_chs = len(self.target_dir)
    ...
    def train(self, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session(config=session_conf) as sess:
            ...
            train_net = DeepFeatureNet(
                ...
                input_chs = self.input_chs,
                n_classes=self.n_classes,
                is_train=True,
                reuse_params=False,
                use_dropout=True,
                n_subjects=self.num_subject,
            ...
            if sess.run(global_step) < n_epochs:
                ...
                for src_dir, tar_dir in zip(self.source_dir, self.target_dir):
                    ...
                    src_data_loaders.append(data_loader)
                    tar_data_loaders.append(unseen_data_loader)

                x_train, y_train, s_train, x_valid, y_valid, s_valid = [], [], [], [], [], []

                for idx, data_loader in enumerate(src_data_loaders):
                    x_tr, y_tr, s_tr, x_val, y_val, s_val= data_loader.load_train_data(oversample=False)
                    if idx >0:
                        a,b = x_tr.shape[0], x_train[idx-1].shape[0]
                        c_tr = min(a,b)
                        if a > b:
                            x_tr = x_tr[:c_tr, :, :, :]
                        else:
                            x_train[idx-1] = x_train[idx-1][:c_tr, :,:,:]

                        a,b = x_val.shape[0], x_valid[idx-1].shape[0]
                        c_val = min(a,b)
                        if a > b:
                            x_val = x_val[:c_val, :, :, :]
                        else:
                            x_valid[idx-1] = x_valid[idx-1][:c_val, :,:,:]
                ...
                unseen_x_train, unseen_y_train, unseen_x_valid, unseen_y_valid = [], [], [], []
                for idx, data_loader in enumerate(tar_data_loaders):
                    unseen_x_tr, unseen_y_tr, unseen_x_val, unseen_y_val = data_loader.load_train_data()
                    if idx > 0:
                        a, b = unseen_x_tr.shape[0], unseen_x_train[idx - 1].shape[0]
                        c_tr = min(a, b)
                        if a > b:
                            unseen_x_tr = unseen_x_tr[:c_tr, :, :, :]
                        else:
                            unseen_x_train[idx - 1] = unseen_x_train[idx - 1][:c_tr, :, :, :]

                        a, b = unseen_x_val.shape[0], unseen_x_valid[idx - 1].shape[0]
                        c_val = min(a, b)
                        if a > b:
                            unseen_x_val = unseen_x_val[:c_val, :, :, :]
                        else:
                            unseen_x_valid[idx - 1] = unseen_x_valid[idx - 1][:c_val, :, :, :]

                    unseen_x_train.append(unseen_x_tr)
                    # y_train.append(y_tr)
                    unseen_x_valid.append(unseen_x_val)
                    # y_valid.append(y_val)

                unseen_x_train = np.concatenate(unseen_x_train, axis=-1)
                if self.input_chs >1:
                    unseen_y_train = unseen_y_tr[:c_tr]
                else:
                    unseen_y_train  = unseen_y_tr
                unseen_x_valid = np.concatenate(unseen_x_valid, axis=-1)
                if self.input_chs >1:

                    unseen_y_valid = unseen_y_val[:c_val]
                else:
                    unseen_y_valid= unseen_y_val
                # Performance history
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session(config=session_conf) as sess:
            ...
            if sess.run(global_step) < n_epochs:
                ...
                src_data_loaders, tar_data_loaders = [], []
                for data_dir, target_dir in zip(self.source_dir, self.target_dir):
                ...
                for idx, data_loader in enumerate(src_data_loaders):
                    x_tr, y_tr, s_tr, x_val, y_val, s_val = data_loader.load_train_data()
                    if idx == 0:
                        x_train = x_tr
                        x_valid = x_val
                        y_train = y_tr
                        ...
                        s_valid = s_val
                    if idx >= 1:
                        for each_idx, x_t  in enumerate(x_tr):
                            a, b = x_t.shape[0], x_train[each_idx].shape[0]
                            c_tr = min(a, b)
                            x_train[each_idx] = np.concatenate([x_train[each_idx][:c_tr], x_t[:c_tr]], axis=-1)
                            y_train[each_idx] = y_train[each_idx][:c_tr]
                            s_train[each_idx] = s_train[each_idx][:c_tr]

                        for each_idx, x_v in enumerate(x_val):
                            a, b = x_v.shape[0], x_valid[each_idx].shape[0]
                            c_val = min(a, b)
                            x_valid[each_idx] = np.concatenate([x_valid[each_idx][:c_val], x_v[:c_val]], axis=-1)
                            y_valid[each_idx] = y_valid[each_idx][:c_val]
                            s_valid[each_idx] = s_valid[each_idx][:c_val]


                unseen_x_train, unseen_y_train, unseen_x_valid, unseen_y_valid = [], [], [], []

                for idx, data_loader in enumerate(tar_data_loaders):
                    x_tr, y_tr, x_val, y_val = data_loader.load_train_data()
                    if idx == 0:
                        unseen_x_train = x_tr
                        unseen_x_valid = x_val
                        unseen_y_train = y_tr
                        unseen_y_valid = y_val
                    if idx >= 1:
                        for each_idx, x_t in enumerate(x_tr):
                            a, b = x_t.shape[0], unseen_x_train[each_idx].shape[0]
                            c_tr = min(a, b)
                            unseen_x_train[each_idx] = np.concatenate([unseen_x_train[each_idx][:c_tr], x_t[:c_tr]], axis=-1)
                            unseen_y_train[each_idx] = unseen_y_train[each_idx][:c_tr]
                        for each_idx, x_v in enumerate(x_val):
                            a, b = x_v.shape[0], unseen_x_valid[each_idx].shape[0]
                            c_val = min(a, b)
                            unseen_x_valid[each_idx] = np.concatenate([unseen_x_valid[each_idx][:c_val], x_v[:c_val]], axis=-1)
                            unseen_y_valid[each_idx] = unseen_y_valid[each_idx][:c_val]
