    ...
    def _run_epoch(self, sess, network, inputs, targets, subjects, inputs_unseen, targets_unseen, train_op, alpha, beta, gamma, is_train):
        ...
        if is_train:
            ...
            for (x_batch, y_batch, s_batch), (x_batch_unseen, y_batch_unseen) in zip(iterate_minibatches_subject(inputs, targets,subjects, self.batch_size, shuffle=is_shuffle), iterate_minibatches(inputs_unseen, targets_unseen, self.batch_size, shuffle=is_shuffle)):
                ...
                _, _, _, _, src_loss_st, src_loss_do, src_loss_cl, src_loss_syn, y_pred = sess.run(
                    [train_op[0], train_op[1], train_op[2], train_op[3],  network.staging_loss,
                     network.domain_loss,
                     network.class_distributor_loss,  network.syn_loss,
                     network.pred_op],
                    feed_dict=feed_dict
                )
            ...
            for (x_batch, y_batch, z_batch) in iterate_minibatches_subject(inputs=inputs, targets=targets, subjects=subjects, batch_size=self.batch_size):
                ...
                _, _, _, _, src_loss_st, src_loss_do, src_loss_cl, src_loss_syn, y_pred = sess.run(
                    [train_op[0], train_op[1], train_op[2], train_op[3], network.staging_loss,
                     network.domain_loss, network.class_distributor_loss,
        ...
        beta = np.mean([src_total_loss_do, src_total_loss_cl]) / src_total_loss_syn
        gamma = src_total_loss_st / (src_total_loss_do +  src_total_loss_cl)
        return total_y_true, total_y_pred, total_y_true_unseen, total_y_pred_unseen, [src_total_loss_st, src_total_loss_do, src_total_loss_cl, src_total_loss_syn], [tar_total_loss_st, tar_total_loss_do, tar_total_loss_cl, tar_total_loss_syn], duration, alpha, beta, gamma
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            train_op5, grads_and_vars_op5 = adam(
                loss=train_net.staging_loss + train_net.syn_loss - (
                            train_net.domain_loss + train_net.class_distributor_loss ),
                lr=self.learning_rate,
            ...
            for epoch in xrange(sess.run(global_step), n_epochs):
                ...
                y_true_train, y_pred_train, y_true_train_unseen, y_pred_train_unseen, src_train_losses, tar_train_losses, train_duration, alpha_temp, beta_temp, gamma_temp = \
                    self._run_epoch(
                        ...
                        train_op=[train_op1, train_op2, train_op4, train_op5],
                ...
                y_true_val, y_pred_val, y_true_val_unseen, y_pred_val_unseen, src_valid_losses, tar_valid_losses, valid_duration, _, _, _= \
                    self._run_epoch(
                        ...
                        train_op=[tf.no_op(), tf.no_op(),  tf.no_op(), tf.no_op()], alpha=1.0, beta=1.0, gamma=1.0,
                        is_train=False
                    )
...
class DeepSleepNetTrainer(Trainer):
    ...
    def _run_epoch(self, sess, network, inputs, targets, subjects, inputs_unseen, targets_unseen, train_op, alpha, beta, is_train):
        ...
        if is_train:
                ...
                for (x_batch, y_batch, s_batch), (x_batch_unseen, y_batch_unseen) in zip(iterate_batch_seq_minibatches_subject(inputs=each_x, targets=each_y,subjects=each_s,
                    ...
                    _, _, _, _, src_loss_st, src_loss_do, src_loss_cl,  src_loss_syn, y_pred = sess.run(
                        [train_op[0], train_op[1], train_op[2], train_op[3],  network.staging_loss, network.domain_loss,
                         network.class_distributor_loss,  network.syn_loss, network.pred_op],
                        feed_dict=feed_dict
                    )
            ...
            for sub_idx, each_data in enumerate(itertools.izip(inputs, targets, subjects)):
                ...
                for (x_batch, y_batch,z_batch) in iterate_batch_seq_minibatches_subject(inputs=each_x, targets=each_y, subjects=each_s, batch_size=self.batch_size, seq_length=self.seq_length):
                    ...
                    _,_,_,_, src_loss_st, src_loss_do, src_loss_cl, src_loss_syn,  y_pred= sess.run(
                        [train_op[0], train_op[1], train_op[2], train_op[3],  network.staging_loss, network.domain_loss, network.class_distributor_loss, network.syn_loss, network.pred_op],
                        feed_dict=feed_dict
                    )
        ...
        return total_y_true, total_y_pred, total_y_true_unseen, total_y_pred_unseen, [src_total_loss_st, src_total_loss_do, src_total_loss_cl,  src_total_loss_syn], [tar_total_loss_st, tar_total_loss_do, tar_total_loss_cl,  tar_total_loss_syn], duration, alpha, beta
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            train_op5, grads_and_vars_op5 = adam(
                loss=train_net.staging_loss + train_net.syn_loss - (train_net.domain_loss + train_net.class_distributor_loss ),
                lr= self.learning_rate,
                train_vars=feature_vars
            )
