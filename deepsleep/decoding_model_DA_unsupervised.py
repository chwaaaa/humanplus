...
class DecodingDeepFeatureNet(object):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            # Softmax linear
            name = "l{}_softmax_linear_stage".format(self.layer_idx)
            ...
            ######### Compute loss #########
            # Cross-entropy loss
            if tf.reduce_sum(self.target_var) !=0:
                ...
                staging_loss = max(self.logits, 0) - self.logits * self.logits + tf.math.log(
                    1 + tf.math.exp(-tf.math.abs(self.logits)))
            ...
            self.losses['regularization loss'] = self.regular_loss
            
            # Cross-entropy loss for domain label
            staging_loss_domain = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits_domain,
                labels=self.domain_var,
                name="sparse_softmax_cross_entropy_with_logits"
            )
            self.domain_loss = tf.reduce_sum(staging_loss_domain) / self.batch_size
            ...
            if 'class' in self.args:
                ...
                for class_idx in range(self.n_classes):
                    ...
                    self.activations.append((name, network))
                    self.layer_idx += 1
                    loss_temp = self.logits[:,class_idx] * tf.nn.sparse_softmax_cross_entropy_with_logits(
                        logits=network,
                        labels=self.domain_var,
                        name="sparse_softmax_cross_entropy_with_logits"
                    )
                    self.class_distributor_loss = tf.add(self.class_distributor_loss, tf.reduce_sum(loss_temp) / self.batch_size)
                self.losses['class loss'] = self.class_distributor_loss
                self.syn_cl_dom_loss = tf.abs(tf.norm(tf.gradients(self.domain_loss, network_ori)) - tf.norm(tf.gradients(self.class_distributor_loss, network_ori)))/self.batch_size
...
class DecodingDeepSleepNet(DecodingDeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            self.domain_loss = tf.contrib.legacy_seq2seq.sequence_loss_by_example(
                [self.logits_domain],
                [self.domain_var],
            ...
            self.losses['staging loss'] =  tf.reduce_mean(staging_loss)
            ...
            if 'class' in self.args:
                ...
                self.syn_cl_dom_loss = tf.abs(tf.norm(tf.gradients(self.domain_loss, network_ori)) - tf.norm(tf.gradients(self.class_distributor_loss, network_ori)))
                self.losses['class loss'] = self.class_distributor_loss
                self.losses['synchronization loss'] = self.syn_cl_dom_loss
